...
const BaseTemplate = (props) => {
  ...
  const allowMultiple =
    props.allowMultiple === undefined ? false : props.allowMultiple;
    ...
    setActivities((prevStae) => {
      ...
      if (index !== -1) {
        tempAct[index] = {
          ...tempAct[index],
          status: status,
          color: status === STATUS_SUCCeSS ? COLOR_SUCCESS : COLOR_ERROR,
          endDate: new Date(),
        };
      }
      return tempAct;
    ...
    <>
      {(baseStage === STAGE_IN_PROGRESS || baseStage === STAGE_FINISHED) && (
        <>
          <BaseTemplateButton
            onClick={resetForm}
            disabled={baseStage === STAGE_IN_PROGRESS}
          />
          <BasetemplateActivities
            activities={activities}
            responseTimeDataset={responseTimeDataset}
          />
        </>
      )}
